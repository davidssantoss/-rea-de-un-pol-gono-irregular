...
class Controlador implements ActionListener {
    ...
    public void actionPerformed(ActionEvent evento) {
        ...
        if (m.x[0] != m.x[m.x.length - 1]) {
            JOptionPane.showMessageDialog(null, "El primer vertice de X debe corresponder con el último vertice de  X");
            return;
        }

        if (m.y[0] != m.y[m.y.length - 1]) {
            JOptionPane.showMessageDialog(null, "El primer vertice de Y debe corresponder con el último vertice de  Y");
            return;
        }
        
        if (evento.getSource()== inter.boton ){
            inter.resul.setText(""+m.calcularArea(m.x,m.y));
            dp = new DibujoPoligono(this);
